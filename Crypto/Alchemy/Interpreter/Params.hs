...
instance ShowType zq => Add_ (Params expr) (CT m zp (Cyc t m' zq)) where
  add_ = showZq @zq "add"
  neg_ = showZq @zq "neg"

instance SingI (p :: Nat) => Add_ (Params expr) (PNoiseCyc ('PN p) t m r) where
  add_ = showPNoise @p "add"
  neg_ = showPNoise @p "neg"

instance SingI (p :: Nat) => AddLit_ (Params expr) (PNoiseCyc ('PN p) t m r) where
  addLit_ _ = showPNoise @p "addLit"

instance ShowType zq => Mul_ (Params expr) (CT m zp (Cyc t m' zq)) where
  type PreMul_ (Params expr) (CT m zp (Cyc t m' zq)) = (CT m zp (Cyc t m' zq))
  mul_ = showZq @zq "mul"

instance SingI (p :: Nat) => Mul_ (Params expr) (PNoiseCyc ('PN p) t m r) where
  type PreMul_ (Params expr) (PNoiseCyc ('PN p) t m r) = 
    PreMul_ expr (PNoiseCyc ('PN p) t m r)
  mul_ = showPNoise @p "mul"

instance SingI (p :: Nat) => Div2_ (Params expr) (PNoiseCyc ('PN p) t m r) where
  type PreDiv2_ (Params expr) (PNoiseCyc ('PN p) t m r) =
    PreDiv2_ expr (PNoiseCyc ('PN p) t m r)

  div2_ = showPNoise @p "div2"

instance SHE_ (Params expr) where

  type ModSwitchPTCtx_   (Params expr) (CT m zp (Cyc t m' zq)) zp' = (ShowType zq)
  type ModSwitchCtx_     (Params expr) (CT m zp (Cyc t m' zq)) zq' = (ShowType zq, ShowType zq')
  type AddPublicCtx_     (Params expr) (CT m zp (Cyc t m' zq)) = (ShowType zq)
  type MulPublicCtx_     (Params expr) (CT m zp (Cyc t m' zq)) = (ShowType zq)
  type KeySwitchQuadCtx_ (Params expr) (CT m zp (Cyc t m' zq)) gad = (ShowType zq)
  type TunnelCtx_        (Params expr) t e r s e' r' s' zp zq gad = (ShowType zq)

  modSwitchPT_ :: forall ct m zp t m' zq zp' env .
    (ModSwitchPTCtx_ (Params expr) ct zp', ct ~ CT m zp (Cyc t m' zq))
    => Params expr env (ct -> CT m zp' (Cyc t m' zq))
  modSwitchPT_     = showZq @zq "modSwitchPT"

  modSwitch_ :: forall ct zq' m zp t m' zq env .
    (ModSwitchCtx_ (Params expr) ct zq', ct ~ CT m zp (Cyc t m' zq))
    => Params expr env (ct -> CT m zp (Cyc t m' zq'))
  modSwitch_       = showZq @zq' $ "modSwitch " ++ showType (Proxy::Proxy zq) ++ " ->"

  addPublic_ :: forall ct m zp t m' zq env .
    (AddPublicCtx_ (Params expr) ct, ct ~ CT m zp (Cyc t m' zq))
    => Cyc t m zp -> (Params expr) env (ct -> ct)
  addPublic_     _ = showZq @zq "addPublic"

  mulPublic_ :: forall ct m zp t m' zq env .
    (MulPublicCtx_ (Params expr) ct, ct ~ CT m zp (Cyc t m' zq))
    => Cyc t m zp -> (Params expr) env (ct -> ct)
  mulPublic_     _ = showZq @zq "mulPublic"

  keySwitchQuad_ :: forall ct gad m zp t m' zq env .
    (KeySwitchQuadCtx_ (Params expr) ct gad, ct ~ CT m zp (Cyc t m' zq))
